...
@media (max-width: 768px) {.table-container { padding: 1rem; } .pengguna-table { font-size: 0.85rem; } .pengguna-table th, .pengguna-table td { padding: 0.75rem 0.5rem; }}
        </style>
</head>
<body>
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <header class="header-bar">
                <h1 class="header-title">Manajemen Level</h1>
            </header>
            ...
            <div class="content-area">
                ...
                <div class="table-container">
                    <div class="table-header">
                        <div class="table-title">Daftar Level</div>
                    </div>
                    @if($levels->count() > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                               <a href="{{ route('level.show', $item->id) }}" class="action-btn view" title="Lihat">
                                            <i data-lucide="eye"></i>
                                        </a>
                                    <a href="{{ route('level.edit', $item->id) }}" class="action-btn edit" title="Edit">
                                        <i data-lucide="edit-3"></i>
                                    </a>
                                    <button type="button" class="action-btn delete" title="Hapus"
                                        onclick="handleDeleteLevel({{ $item->id }}, '{{ addslashes($item->nama) }}')">
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <h3 style="margin-bottom: 0.5rem;">Belum ada level</h3>
                            <p>Mulai dengan menambahkan level baru.</p>
                        </div>
                    @endif
                </div>
                ...
                @if($levels->hasPages())
                    <div class="pagination">
                        ...
                        @else
                            <a href="{{ $levels->previousPageUrl() }}" class="pagination-btn">‹</a>
                        @endif
                        ...
                        @if($levels->hasMorePages())
                            <a href="{{ $levels->nextPageUrl() }}" class="pagination-btn">›</a>
                        @else
                            <button class="pagination-btn" disabled>›</button>
                        @endif
                    </div>
    ...
    <script>
        // Handle Delete Level
        function handleDeleteLevel(id, nama) {
            showModal({
                type: 'delete',
                title: 'Hapus Level',
                message: `Apakah Anda yakin ingin menghapus level "${nama}"? Tindakan ini tidak dapat dibatalkan dan semua materi yang menggunakan level ini akan kehilangan level-nya.`,
                icon: 'trash-2',
                ...
                onConfirm: function() {
                    ...
                    form.method = 'POST';
                    form.action = `/dashboard/level/${id}`;
                    ...
                    if (typeof showInfoToast !== 'undefined') {
                        showInfoToast('Menghapus...', 'Sedang menghapus level...');
                    }
